...
void kplge::PageAllocator::Reset(size_t data_size, size_t page_size,
  ...
  size_t minimal_size =
      (sizeof(BlockHeader) > m_szDataSize) ? sizeof(BlockHeader) : m_szDataSize;
...
void* kplge::PageAllocator::Allocate() {
  if (!m_pFreeList) {
    // allocate a new page
    PageHeader* pNewPage =
...
void kplge::PageAllocator::Free(void* p) {
...
void kplge::PageAllocator::FreeAll() {
...
#if defined(KPL_DEBUG)
void kplge::PageAllocator::FillFreePage(PageHeader* pPage) {
  // page header
  pPage->pNext = nullptr;
...
void kplge::PageAllocator::FillFreeBlock(BlockHeader* pBlock) {
  // block header + data
...
void kplge::PageAllocator::FillAllocatedBlock(BlockHeader* pBlock) {
  // block header + data
